...
void sgf::render::pipeline::execute(base::renderer &rd, resource::manager *const mgr)
{
    ...
    auto flush                       = [&]()
    {
        ...
        rd.render_geometry(current_tex,
                           vertex_buffer.data(), static_cast<sgf::type::int32>(vertex_buffer.size()),
                           index_buffer.data(), static_cast<sgf::type::int32>(index_buffer.size()));
        vertex_buffer.clear();
        index_buffer.clear();
    };
    ...
    {
        base::texture *cmd_tex = (mgr && cmd.texture_key != 0) ? mgr->get<base::texture>(cmd.texture_key) : nullptr;

        if (cmd_tex != current_tex)
        {
            flush();
            current_tex = cmd_tex;
        }
        ...
        sgf::type::colorf col = (cmd.texture_key != 0 && cmd.color.a == 0) ? sgf::type::colorf{1.0f, 1.0f, 1.0f, 1.0f} : sgf::type::colorf{cmd.color};
        ...
        if (cmd_tex)
        {
            ...
            if (tw > 0 && th > 0)
            {
                u0 = cmd.src.x / (float)tw;
                v0 = cmd.src.y / (float)th;
                u1 = (cmd.src.x + cmd.src.w) / (float)tw;
                v1 = (cmd.src.y + cmd.src.h) / (float)th;
            }
        }

        if (cmd.style == 1)
        {
            if (cmd.submission_type == render::type::texture || cmd.submission_type == render::type::rect)
            {
                ...
                float w  = final_dst.w;
                float h  = final_dst.h;
                float px = cmd.pivot.x * w;
                float py = cmd.pivot.y * h;

                float x_coords[4] = {-px, w - px, -px, w - px};
                float y_coords[4] = {-py, -py, h - py, h - py};
                float u_coords[4] = {u0, u1, u0, u1};
                float v_coords[4] = {v0, v0, v1, v1};

                for (int i = 0; i < 4; ++i)
                {
                    float rx = x_coords[i] * cos_a - y_coords[i] * sin_a + final_dst.x;
                    float ry = x_coords[i] * sin_a + y_coords[i] * cos_a + final_dst.y;
                    vertex_buffer.push_back({{rx, ry}, col, {u_coords[i], v_coords[i]}});
                }

                index_buffer.insert(index_buffer.end(), {base_idx + 0, base_idx + 1, base_idx + 2,
